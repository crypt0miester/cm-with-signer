...
export const mintOneToken = async (
  ...
  let seed = new Uint8Array(JSON.parse(process.env.REACT_APP_CANDY_SIGNER!));
  const candyMachineSigner = anchor.web3.Keypair.fromSecretKey(seed);
  const candyMachineAddress = candyMachine.id;
  const remainingAccounts = [];
  const cleanupInstructions = [];
  const instructions = [];
  const signers: anchor.web3.Keypair[] = [];
  signers.push(candyMachineSigner);
  ...
  instructions.push(
    await candyMachine.program.instruction.mintNft(creatorBump, {
      accounts: {
        ...
        candyMachineCreator,
        candyMachineSigner:candyMachineSigner.publicKey,
        payer: payer,
