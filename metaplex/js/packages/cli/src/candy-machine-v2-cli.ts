...
programCommand('upload')
  ...
  .option(
    '-cms, --candy-machine-signer <string>',
    'optional use a candy machine signer, provide JSON file to keypair',
  )
  .action(async (files: string[], options, cmd) => {
    ...
    const candyMachineSignerKeyPair = loadWalletKey(candyMachineSigner) || walletKeyPair;
    ...
    if (mintSignerRequired && candyMachineSignerKeyPair === undefined) {
      throw new Error(
    ...
    if (mintSignerRequired && candyMachineSignerKeyPair === walletKeyPair) {
      log.info(
        'WARNING: You will have to use the walletKeyPair on the frontend. use at own risk, we do not recommend this!',
      );
    }
...
programCommand('show')
  ...
  .action(async (directory, cmd) => {
    ...
    try {
      ...
      //@ts-ignore
      log.info('mintSigner: ', machine.mintSigner.toBase58());
      if (collectionPDAAccount) {
      ...
      //@ts-ignore
      log.info('isSignatureRequired: ', machine.data.isSignatureRequired);
      //@ts-ignore
...
programCommand('update_candy_machine')
  ...
  .action(async (directory, cmd) => {
    ...
    const newSettings = {
      ...
      isMutable: mutable,
      isSignatureRequired: mintSignerRequired,
...
programCommand('mint_one_token')
  ...
  .action(async (directory, cmd) => {
    const { keypair, env, cacheName, rpcUrl, candyMachineSigner } = cmd.opts();

    const cacheContent = loadCache(cacheName, env);
    const candyMachine = new PublicKey(cacheContent.program.candyMachine);
    const tx = await mintV2(keypair, env, candyMachine, rpcUrl, candyMachineSigner);
...
programCommand('mint_multiple_tokens')
  ...
  .action(async (_, cmd) => {
    ...
    const mintToken = async index => {
      const tx = await mintV2(keypair, env, candyMachine, rpcUrl, "2");
