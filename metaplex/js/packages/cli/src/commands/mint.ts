...
export async function mintV2(
  ...
  rpcUrl: string,
  candyMachineSigner: string,
): Promise<string> {
  const mint = Keypair.generate();
  console.log(candyMachineSigner)
  const userKeyPair = loadWalletKey(keypair);
  const candyMachineSignerKeyPair = loadWalletKey(candyMachineSigner) || userKeyPair;
  ...
  const remainingAccounts = [];
  const signers = [mint, userKeyPair, candyMachineSignerKeyPair];
  ...
  instructions.push(
    await anchorProgram.instruction.mintNft(creatorBump, {
      accounts: {
        candyMachine: candyMachineAddress,
        candyMachineCreator,
        candyMachineSigner: candyMachineSignerKeyPair.publicKey,
        payer: userKeyPair.publicKey,
        //@ts-ignore
        wallet: candyMachine.wallet,
        mint: mint.publicKey,
