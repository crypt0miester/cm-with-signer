...
anchor_lang::declare_id!("CnDYpzubbYDigGfMpDKdy23dqUuVhsALqtf2FTX7EgqL");
...
pub mod candy_machine {
    ...
    pub fn mint_nft<'info>(
        ...
        let candy_machine_signer = &ctx.accounts.candy_machine_signer;
        ...
        // checking if signature for the minting is required.
        // else no checking of the candy_machine_signer(cms) is needed
        if candy_machine.data.is_signature_required {
            if let ms = candy_machine_signer {
                match ms.key() {
                    // val is what ever the MintNFT passes as the cms
                    val => {
                        // msg!("{}", val);
                        if (candy_machine.authority == val) {
                            // authority can do whatever they like I guess.
                            msg!("Authority is signer, so is ok.");
                        }
                        // punish if the val is not the cms.
                        if (candy_machine.mint_signer != val) {
                            msg!("Signer is not the original mint_signer");
                            if ctx.accounts.payer.key() != candy_machine.authority {
                                punish_bots(
                                    ErrorCode::MintingNotSigned,
                                    payer.to_account_info(),
                                    ctx.accounts.candy_machine.to_account_info(),
                                    ctx.accounts.system_program.to_account_info(),
                                    BOT_FEE,
                            ...
                            return Err(ErrorCode::MintingNotSigned.into());
                        }
                        // everything looks ok, continue
                        // msg!("Signer is the original cms. continue.");
                    }
                    _ => {
                            if ctx.accounts.payer.key() != candy_machine.authority {
                                punish_bots(
    ...
    pub fn update_mint_signer(
        ctx: Context<UpdateCandyMachine>,
        mint_signer: Option<Pubkey>,
    ) -> ProgramResult {
        let candy_machine = &mut ctx.accounts.candy_machine;

        if let Some(new_mint_signer) = mint_signer {
            candy_machine.mint_signer = new_mint_signer;
        }
...
pub struct InitializeCandyMachine<'info> {
    ...
    // mint_signer can be any pubkey that the authority has keys to
    // if not provided it should fall back to authority.
    // #[account(constraint = candy_machine.to_account_info().authority || mint_signer.is_signer)]
    /// CHECK: maybe we need to check for something here? 
    mint_signer: Signer<'info>,
...
pub struct MintNFT<'info> {
    ...
    // the cm creator may not want to use the feature thus, if candy_machine_signer == authority
    // perhaps the MintNFT should assume that this is not a signed candy machine mint rather
    // it is a regular MintNFT
    // #[account(constraint = candy_machine.to_account_info().authority || mint_signer.is_signer)]
    /// CHECK: maybe we need to check for something here? 
    candy_machine_signer: Signer<'info>,
    payer: Signer<'info>,
    /// CHECK: wallet can be any account and is not written to or read
    #[account(mut)]
    wallet: UncheckedAccount<'info>,
